...
function mapFinancialProduct(row: FinancialProductRow): FinancialProduct {
  return {
    id: row.id,
    name: row.name,
    type: row.type,
    ...
    maximumAmount:
      row.maximum_amount === null ? null : Number(row.maximum_amount),
    minimumTermMonths: row.minimum_term_months,
    maximumTermMonths: row.maximum_term_months,
...
export class FinancialProductRepository {
  ...
  async findByType(type: string): Promise<FinancialProduct[]> {
    const result = await pool.query<FinancialProductRow>(
      `SELECT * FROM financial_products WHERE type = $1`,
      [type],
    );
    return result.rows.map(mapFinancialProduct);
  }
